...
| 采集任务 `ImageCollectionTask` | 用户定义的一次采集意图或一段定时采集计划 |
| 采集执行 `ImageCollectionExecution` | 一个具体计划时间点的实际执行事实 |
...
| `voglander-service` | 上传接入、采集执行、调度、媒体会话和存储编排 | 不保存 Repository DO，不实现协议细节 |
| `voglander-manager` | DTO/DO 转换、状态转换、分页查询、事务、缓存清理 | 公开接口不返回 DO，不直接调用 ZLM |
...
### 8.3 `tb_image_collection_task`

| 字段 | 逻辑类型 | 约束/说明 |
| --- | --- | --- |
| `id` | BIGINT | 主键 |
| `task_id` | VARCHAR(64) | 对外稳定 ID，唯一 |
| `task_name` | VARCHAR(255) | 展示名称 |
| `collection_mode` | VARCHAR(32) | `ONCE/SCHEDULED` |
| `state` | VARCHAR(32) | 任务状态 |
| `camera_id` | VARCHAR(64) | 稳定资源 ID 预留 |
| `device_id` | VARCHAR(64) | 必填事实快照 |
| `channel_id` | VARCHAR(64) | 必填事实快照 |
| `device_name_snapshot` | VARCHAR(255) | 创建时名称 |
| `channel_name_snapshot` | VARCHAR(255) | 创建时名称 |
| `schedule_start_time` | DATETIME | 定时任务必填 |
| `schedule_end_time` | DATETIME | 定时任务必填 |
| `interval_seconds` | INTEGER | 定时任务必填 |
| `next_execute_time` | DATETIME | 调度索引 |
| `schedule_version` | INTEGER | 调度版本，默认 1 |
| `planned_count` | INTEGER | 创建时预计数量 |
| `success_count` | INTEGER | 运行统计 |
| `failed_count` | INTEGER | 运行统计 |
| `missed_count` | INTEGER | 运行统计 |
| `last_execute_time` | DATETIME | 最近实际执行时间 |
| `last_failure_code` | VARCHAR(64) | 最近失败摘要 |
| `last_failure_message` | VARCHAR(512) | 脱敏信息 |
| `owner_id` | VARCHAR(64) | 创建用户 |
| `organization_id` | VARCHAR(64) | 组织隔离预留 |
| `idempotency_key` | VARCHAR(128) | 创建任务请求幂等键 |
| `retention_policy` | VARCHAR(64) | 生成资产继承，第一阶段 `PERMANENT` |
| `version` | INTEGER | 乐观锁 |
| `create_time` | DATETIME | 创建时间 |
| `update_time` | DATETIME | 更新时间 |
...
- `UNIQUE(task_id)`
- `INDEX(state, next_execute_time)`
- `INDEX(device_id, channel_id, create_time)`
- `INDEX(owner_id, create_time)`
- `UNIQUE(owner_id, idempotency_key)`，防止重复创建任务

### 8.4 `tb_image_collection_execution`

| 字段 | 逻辑类型 | 约束/说明 |
| --- | --- | --- |
| `id` | BIGINT | 主键 |
| `execution_id` | VARCHAR(64) | 对外稳定 ID，唯一 |
| `task_id` | VARCHAR(64) | 归属任务 |
| `schedule_version` | INTEGER | 执行时任务版本 |
| `planned_at` | DATETIME | 计划时间 |
| `started_at` | DATETIME | 开始时间 |
| `captured_at` | DATETIME | 实际截图时间 |
| `finished_at` | DATETIME | 结束时间 |
| `state` | VARCHAR(32) | 执行状态 |
| `asset_id` | VARCHAR(64) | 成功时关联资产 |
| `attempt_count` | INTEGER | 尝试次数 |
| `failure_code` | VARCHAR(64) | 稳定失败码 |
| `failure_message` | VARCHAR(512) | 脱敏失败信息 |
| `worker_node` | VARCHAR(128) | 执行节点 |
| `version` | INTEGER | 乐观锁 |
| `create_time` | DATETIME | 创建时间 |
| `update_time` | DATETIME | 更新时间 |
...
任务最多生成一个资产。执行记录仍然存在，以保持单次与定时查询结构一致。
...
| --- | --- |
| 所有计划点成功 | `COMPLETED` |
| 至少一个成功，且存在失败或错过 | `PARTIAL_COMPLETED` |
| 无成功，且所有可执行点均失败 | `FAILED` |
| 用户取消 | `CANCELLED` |
...
`MISSED` 表示计划点已经失去真实采集意义，不进入截图执行队列。
...
```mermaid
sequenceDiagram
    participant SCH as 调度/单次入口
    participant EXE as ImageCollectionExecutor
    ...
    participant STORE as ImageStorageService
    participant MAN as Image Asset/Execution Manager

    SCH->>MAN: 创建 PENDING execution
    EXE->>MAN: PENDING → RUNNING
    EXE->>DEV: 校验设备、通道与协议
    EXE->>REG: 查询可复用直播会话
    alt 已有可用流
...
- `image.asset.created`
- `image.asset.deleted`
- `image.collection.task-state`
- `image.collection.execution-state`
...
- 图像来源、资产状态、采集模式、任务状态、执行状态、存储类型枚举。
- 图像错误码和缓存/锁常量。

### 19.2 `voglander-client`

- `ImageAssetDTO`、`ImageAssetSourceDTO`。
- `ImageCollectionTaskDTO`、`ImageCollectionExecutionDTO`。
- 上传、采集、调度和查询 command/query DTO。
- `ImageStorageService`、`MediaSnapshotAdapter` 等稳定 SPI。

### 19.3 `voglander-repository`

- 四张表的 DO、Mapper 和必要 XML。
- 调度扫描、状态条件更新和分页查询。
- 不缓存图像二进制。
...
- `ImageCollectionTaskManager`。
- `ImageCollectionExecutionManager`。
...
- `ImageCollectionService`：任务命令门面。
- `ImageCollectionScheduler`：多节点调度扫描。
- `ImageCollectionExecutor`：单点截图执行。
- `ImageAssetLifecycleService`：手动删除和删除重试。
- 独立 `imageCollectionExecutor` 线程池。
...
- 采集任务和执行记录 Controller、Req、VO、Assembler。
- 图像内容流响应和权限校验。
- OpenAPI 注解。
