-- Voglander 1.0.9 image asset collection migration (SQLite)
-- Requires add-durable-business-task-engine and contains no DROP statements.
...
CREATE TABLE IF NOT EXISTS tb_image_asset (
    ...
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, asset_id VARCHAR(64) NOT NULL,
    ...
    captured_at DATETIME NOT NULL, ingested_at DATETIME NOT NULL, owner_type VARCHAR(32) NOT NULL,
    owner_id VARCHAR(64) NOT NULL, organization_id VARCHAR(64), idempotency_key VARCHAR(128),
    retention_policy VARCHAR(64) NOT NULL DEFAULT 'PERMANENT', expires_at DATETIME, deleted_at DATETIME,
    delete_reason VARCHAR(255), failure_code VARCHAR(64), failure_message VARCHAR(512), version INTEGER NOT NULL DEFAULT 0
);
CREATE UNIQUE INDEX IF NOT EXISTS uk_image_asset_asset_id ON tb_image_asset(asset_id);
CREATE UNIQUE INDEX IF NOT EXISTS uk_image_asset_idempotency ON tb_image_asset(owner_type,owner_id,idempotency_key);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_image_asset_source_asset ON tb_image_asset_source(asset_id);
CREATE UNIQUE INDEX IF NOT EXISTS uk_image_asset_source_execution ON tb_image_asset_source(source_execution_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_image_collection_config_task ON tb_image_collection_config(task_id);
...
(700,0,'ImageManagement','image.management.title',1,'/image','BasicLayout','lucide:images',7,1,'','{"icon":"lucide:images","title":"image.management.title","order":7}'),
(701,700,'ImageAssets','image.asset.title',2,'/image/assets','/image/assets/list','lucide:image',1,1,'Image:Asset:Query','{"icon":"lucide:image","title":"image.asset.title"}'),
(702,700,'ImageCollection','image.collection.title',2,'/image/collection','/image/collection/list','lucide:camera',2,1,'Image:Collection:Query','{"icon":"lucide:camera","title":"image.collection.title"}'),
(70101,701,'ImageAssetQuery','image.asset.query',3,NULL,NULL,'',1,1,'Image:Asset:Query','{"title":"image.asset.query","hideInMenu":true}'),
(70102,701,'ImageAssetView','image.asset.view',3,NULL,NULL,'',2,1,'Image:Asset:View','{"title":"image.asset.view","hideInMenu":true}'),
(70103,701,'ImageAssetUpload','image.asset.upload',3,NULL,NULL,'',3,1,'Image:Asset:Upload','{"title":"image.asset.upload","hideInMenu":true}'),
(70104,701,'ImageAssetDownload','image.asset.download',3,NULL,NULL,'',4,1,'Image:Asset:Download','{"title":"image.asset.download","hideInMenu":true}'),
(70105,701,'ImageAssetDelete','image.asset.delete',3,NULL,NULL,'',5,1,'Image:Asset:Delete','{"title":"image.asset.delete","hideInMenu":true}'),
(70201,702,'ImageCollectionQuery','image.collection.query',3,NULL,NULL,'',1,1,'Image:Collection:Query','{"title":"image.collection.query","hideInMenu":true}'),
(70202,702,'ImageCollectionCreate','image.collection.create',3,NULL,NULL,'',2,1,'Image:Collection:Create','{"title":"image.collection.create","hideInMenu":true}'),
(70203,702,'ImageCollectionControl','image.collection.control',3,NULL,NULL,'',3,1,'Image:Collection:Control','{"title":"image.collection.control","hideInMenu":true}');
