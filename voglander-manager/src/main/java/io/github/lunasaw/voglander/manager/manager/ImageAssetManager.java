...
public class ImageAssetManager {
    ...
    public ImageAssetCreateResultDTO createWithSource(ImageAssetDTO asset, ImageAssetSourceDTO source) {
        ...
        if (assetMapper.insertIfAbsent(assetDO) == 0) {
            ImageAssetDTO winner = StringUtils.hasText(asset.getIdempotencyKey())
                ? findByIdempotency(asset.getOwnerType(), asset.getOwnerId(), asset.getIdempotencyKey()) : null;
            if (winner == null) {
                throw new IllegalStateException("image asset insert was ignored without an idempotency winner");
            }
            return accepted(false, winner);
        }
        ...
        ImageAssetSourceDO acceptedSource = sourceDO;
        if (sourceMapper.insertIfAbsent(sourceDO) == 0) {
            acceptedSource = sourceMapper.selectByAssetId(source.getAssetId());
            if (acceptedSource == null) throw new IllegalStateException("image source insert was ignored without an existing source");
        }
        ImageAssetDTO accepted = ImageAssetAssembler.toDTO(assetDO);
        publishAssetEvent(accepted, "ASSET_CREATED");
        return new ImageAssetCreateResultDTO(true, accepted, ImageAssetAssembler.toDTO(acceptedSource));
    }

    private ImageAssetCreateResultDTO accepted(boolean created, ImageAssetDTO asset) {
        ImageAssetSourceDTO source = getSourceByAssetId(asset.getAssetId());
        if (source == null) {
            throw new IllegalStateException("accepted image asset has no source");
        }
        return new ImageAssetCreateResultDTO(created, asset, source);
    }
    ...
    public Page<ImageAssetEnrichedDTO> getEnrichedPage(ImageAssetQueryDTO query, long page, long size) {
        Assert.isTrue(page > 0, "page必须大于0");
        Assert.isTrue(size > 0 && size <= 1000, "size必须在1-1000之间");
        Assert.notNull(query, "query不能为空");
        Page<ImageAssetWithSourceDO> source = assetMapper.selectEnrichedPageByCondition(
            new Page<ImageAssetWithSourceDO>(page, size), query.getOwnerType(), query.getOwnerId(),
            query.getAssetId(), query.getAssetName(), query.getStatus(), query.getSourceType(),
            query.getSourceTaskId(), query.getSourceExecutionId(), query.getDeviceId(), query.getChannelId(),
            query.getCapturedStart(), query.getCapturedEnd());
        Page<ImageAssetEnrichedDTO> target = new Page<ImageAssetEnrichedDTO>(page, size, source.getTotal());
        target.setPages(source.getPages());
        target.setRecords(source.getRecords().stream().map(ImageAssetAssembler::toDTO)
            .collect(Collectors.toList()));
        return target;
    }

    public ImageAssetEnrichedDTO getEnrichedDetail(String assetId) {
        Assert.hasText(assetId, "assetId不能为空");
        return ImageAssetAssembler.toDTO(assetMapper.selectEnrichedByAssetId(assetId));
    }
    ...
    private void publishAssetEvent(ImageAssetDTO asset, String eventType) {
        ...
        String topic;
        if ("ASSET_DELETING".equals(eventType)) {
            topic = ImageConstant.SSE_ASSET_DELETING;
        } else if ("ASSET_DELETED".equals(eventType) || "ASSET_DELETE_FAILED".equals(eventType)) {
            topic = ImageConstant.SSE_ASSET_DELETED;
        } else {
            topic = ImageConstant.SSE_ASSET_CREATED;
        }
