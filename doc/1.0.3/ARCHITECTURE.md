...
> 适用代码基线：sip-gateway 1.8.0 接入完成 + GB28181 出站命令 envelope 化改造完成
...
| 链路 | 入口 | 关键组件 | 出口 |
|------|------|----------|------|
| **GB28181 设备接入** | 设备 SIP 报文 | sip-gateway → `VoglanderBusinessNotifier` → `DeviceRegisterService`/`DeviceManager`/`MediaSessionManager` | 数据库设备/通道/会话状态 |
| **GB28181 平台指令** | REST/业务调用 | `VoglanderServerXxxCommand` → `CommandHandlerRegistry`(envelope) → SIP 下发 | 设备执行（PTZ/点播/回放/配置） |
...
| 公共库 | luna-common | 2.6.7-SNAPSHOT | `ResultDTO`/`ResultDTOUtils`/工具集 |
...
| client | 服务契约与传输模型 | `DeviceRegisterService` 接口、`DeviceRegisterReq`/`DeviceChannelReq`/`DeviceInfoReq` |
| repository | 持久化与缓存 | 14 个 `*DO`、15 个 `*Mapper`、`RedisCache`、`RedisLockUtil`、`@Cached`/`CachedAspect` |
| manager | 业务编排 + 模板方法 | 13 个 `*Manager`、13 个 `*Service`/Impl、`*Assembler`、`AsyncManager`、`ThreadPoolConfig` |
| service | 领域服务 | `DeviceRegisterServiceImpl`、`StreamProxyBizService`、`DeviceAgreementService` |
| integration | 外部系统包装 | GB28181 notifier/command/store/supplier、ZLM wrapper/hook、EasyExcel、IP |
...
> 本节是 1.0.3 相对历史版本变化最大的部分。GB28181 已从「直接 handler 分发」迁移到 sip-gateway 1.8.0 的「**出站命令 envelope + 入站统一回调**」双通道模型。`wrapper/gb28181/` 下不再有任何 `*RequestHandler`/`*ResponseHandler`。
...
├── notifier/
│   └── VoglanderBusinessNotifier      # 入站唯一入口（implements BusinessNotifier）
├── server/command/                    # 平台端出站命令（envelope 化）
│   ├── AbstractVoglanderServerCommand # 抽象基类：dispatchEnvelope 模板
│   ├── ptz/ record/ device/ alarm/ catalog/ status/ config/ media/
...
### 7.2 入站：统一回调 `VoglanderBusinessNotifier`

**唯一入口**，直接实现框架 `BusinessNotifier`（覆盖默认 `NoopBusinessNotifier`，后者 `@ConditionalOnMissingBean`）。
...
VoglanderBusinessNotifier.notify(GatewayEvent)   @Async("sipNotifierExecutor")
   │
   ▼ switch(event.type())  三段式 gb28181.Group.Name
   ├── Lifecycle.Register          → DeviceRegisterService.login(DeviceRegisterReq)
   ├── Lifecycle.Online            → DeviceManager.updateStatus(deviceId, ONLINE)
...
- `notify(GatewayEvent)` **必须异步**（`@Async("sipNotifierExecutor")`），否则设备端 SIP 超时重传；
- **不能**继承 `AbstractProtocolBusinessNotifier`——其 `notify()` 为 `final` 且自调用 `onProtocolEvent`，`@Async` 代理失效；
- `GatewayEvent` 是 record：`(type, deviceId, correlationId, timestampMs, payload:Map, nodeId)`；
- payload 一律用 **FastJSON2** 反序列化为对应实体（`DeviceKeepLiveNotify`/`DeviceResponse`/`DeviceInfo`/`MediaStatusNotify`）。
...
各业务域命令（ptz/record/device/alarm/catalog/status/config/media）均已改走 envelope（见近期 commit `499be7f`~`a855f86`）。客户端命令 `ClientCommandSender` 保持 `@Component` + 静态方法，7 个设备端命令大体不变（PTZ 改用 `PTZControlEnum`+`PTZInstructionBuilder`）。
...
- 由 Notifier 的 `Session.*` 与 `Notify.MediaStatus` 事件驱动：`onInviteOk`/`onInviteFailure`/`onAck`/`onBye`/`onMediaStatus`；
...
| `sipNotifierExecutor` | core=8 / max=32 / queue=1000，CallerRuns | **GB28181 事件回调专用**（`VoglanderBusinessNotifier.notify`） |

- `@EnableAsync` 开启；`sipNotifierExecutor` 满载时 CallerRuns 回退，保证回调不丢；
- `AsyncManager` 处理通用后台任务；
- 消息队列：RabbitMQ（可选 RocketMQ）。

### 12.2 异步设计要点

- GB28181 `notify` 必须异步，否则设备 SIP 超时重传（见 §7.2）；
...
- **多节点集群**：`store.type=redis`，各节点唯一 `node-id`，依赖 Redis 做 INVITE 跨节点回���路由；前端经负载均衡接入。
...
| 2 | `VoglanderBusinessNotifier.notify` 必须 `@Async` 且不继承 `AbstractProtocolBusinessNotifier` | 同步 → 设备 SIP 超时重传；继承基类 → `final notify()` 致 `@Async` 失效 |
...
| 9 | 根 pom ��� `spring-boot-dependencies` 必须在 `sip-gateway-bom` **之前** import | 顺序颠倒 → Spring Boot 被降级到 3.3.1 → `@MockitoBean`（需 Spring 6.2+）消失，测试编译失败 |
...
- ✅ GB28181 从 gb28181 1.2.5-SNAPSHOT 直连 handler 迁移到 sip-gateway 1.8.0；
- ✅ 入站 35 种 `GatewayEvent` 统一回调（`VoglanderBusinessNotifier`）；
- ✅ 出站 8 个业务域命令全部 envelope 化（`dispatchEnvelope`）；
- ✅ INVITE 会话状态建模（`MediaSessionManager` + `tb_media_session`）；
